...
#include "nodeType.h"
template <class Type>
class unorderedLinkedList : protected linkedListType<Type>
{
public:
	...
	virtual void insertFirst(const Type& newItem);
	//Function to insert newItem t the beginning of the list.
	//Postcondition: first points to the new list, newItem is inserted
	//     at the beginning of the list, last points to the last node, 
	//     and count is incremented by 1. 

	virtual void insertLast(const Type& newItem);
	//Function to insert newItem at the end of the list.
	//Postcondition: first points to the new list, 
	//    newItem is inserte4d at the end of the list, 
	//    last points to the last node, and count is incremented by 1.

	virtual void deleteNode(const Type& deleteItem);
	//Function to delete deleteItem from the list.
	//Postcondition: if found, the node containing deleteItem is deleted from
	//    the list. first points to the first node, last points to the last 
...
bool unorderedLinkedList<Type>::search(const Type& searchItem) const
{
	...
	current = this->first; //set current point to the frist node in the list
...
void unorderedLinkedList<Type>::insertFirst(const Type& newItem)
{
	...
	newNode->link = this->first; //insert newNode b efore first
...
{
	nodeType<Type> *current; //pointer to traverse the list
	nodeType<Type> *trailCurent; //pointer just before current
	bool found;
	...
	else
	{
		...
		{
			found = false;
			nodeType<Type>* trailCurrent = this->first; //set trailCurrent to point to the first node
			...
			if (found) // case 3; if found, delete the node
			{
				...
				delete current; //delte teh node from the list
			}
			else
